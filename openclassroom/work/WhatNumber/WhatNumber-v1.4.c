...
#include <time.h>
    ...
    /*        quand je découvre de nouvelles chose dans mon cours de C, alors bienvenue dans    */
    ...
    /*        plus mais ce n'est pas l'intérêt, je veux y mettre tout ce que j'apprend pour     */
    /*        m'entrainer ou juste que je trouve cool!)                                         */
    /********************************************************************************************/
...
int     newRecord(int newScore)        //fonction de nouveau record
{
    ...
    if (scoreBoard != NULL)  
    {
        ...
        while (score[index]) //on parcours le tableau
        {
            if (newScore < score[index]) //on cherche un score supérieur au nouveau
                {
                    ...
                    printf("les nouveaux records sont : %d, %d et %d\n\n", score[0], score[1], score[2]);
...
int     main(int argc, char **argv)  //on initialise le main
{
    ...
    printf("bonjour et bienvenue dans le What Number ! \n\n");
